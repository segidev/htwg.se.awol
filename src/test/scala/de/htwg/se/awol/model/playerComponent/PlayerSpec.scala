...
class PlayerSpec extends WordSpec with Matchers {
  "A new Player" should {
    "be the Pöbel when initialized without a rank" in {
      ...
      player.rank should be(PlayerEnvironment.P_Mob)
  ...
  "A Player" can {
    ...
    "change it's rank to be a King and should be the König now" in {
      player.setRank(PlayerEnvironment.P_King)
      player.rank should be(PlayerEnvironment.P_King)
      player.rankName should be("König")
    }
    "also become an Asshole what makes him an Arschloch" in {
      player.setRank(PlayerEnvironment.P_Asshole)
      player.rank should be(PlayerEnvironment.P_Asshole)
